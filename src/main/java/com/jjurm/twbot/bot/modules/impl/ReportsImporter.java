...
public class ReportsImporter extends AbstractModule {
	...
	void processReport(HtmlPage page, Connection con) throws SQLException {
		...
		HtmlElement unit_out = page
				.getFirstByXPath("//*[@id='attack_spy']/tbody/tr/td[@colspan='2']/table[contains(@class,'vis')]/tbody/tr[2]");
		...
		try (PreparedStatement stmt_unit_home = con.prepareStatement(query1);
			...
			// ignore - no visible defender's units
		} catch (SQLException e) {
			e.printStackTrace();
		}

		spy_result: try { // ===== Spy result =====
			HtmlElement attack_spy = page.getFirstByXPath("//*[@id='attack_spy']");
			if (attack_spy == null)
				break spy_result;

			resources: try { // Resources
				HtmlElement res = attack_spy.getFirstByXPath("./tbody/tr[1]/td");
				if (res == null)
					break resources;
				HtmlElement el;
				DomText txtNode;
			...
			building: try { // Buildings
				HtmlElement buildings = attack_spy.getFirstByXPath("./tbody/tr[2]/td");
				if (buildings == null)
					break building;
				...
				try (PreparedStatement stmt = con.prepareStatement(sql1);
					...
					Arrays.fill(production, bot.getTWEnvironment().getProduction(0));

					for (int i = 0; i < TWEnvironment.buildings.length; i++) {
						String b = TWEnvironment.buildings[i];
						String buildingName = Config.buildingNames.get(b);
						DomText building = buildings.getFirstByXPath("./text()[contains(., '"
								+ buildingName + "')]");
						int level = 0;
						if (building != null) {
							HtmlElement el = buildings.getFirstByXPath("./text()[contains(., '"
									+ buildingName + "')]/following-sibling::b[1]");
							String s = el.getTextContent().trim();
							Pattern pattern = Pattern.compile(" ([0-9]+)\\)");
							Matcher matcher = pattern.matcher(s);
							matcher.find();
							String strLevel = matcher.group(1);
							level = Integer.parseInt(strLevel);
						}
						stmt.setInt(i + 3, level);
						int index = resources.indexOf(b);
						if (index != -1) {
							production[index] = bot.getTWEnvironment().getProduction(level);
						}
					}

					for (int i = 0; i < 3; i++) {
						stmt_prod.setDouble(i + 3, production[i]);
					}
			...
			// ignore - means no spy result available
		}
